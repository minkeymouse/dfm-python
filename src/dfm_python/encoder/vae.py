...
if _has_torch:
    ...
    class BlockSparseDecoder(nn.Module):
        """Block-sparse linear decoder network for DDFM with block structure.
        
        Maps latent factors f_t back to observed variables X_t using a linear transformation
        with block structure constraints. Series can only load on factors from blocks they
        belong to, enforcing the same block structure as linear DFM.
        
        The decoder's weight matrix C (N x m) is constrained so that:
        - Series i can only load on factors from blocks where series.blocks[j] = 1
        - This creates a block-sparse loading matrix similar to linear DFM
        ...
        output_dim : int
            Number of series (output dimension)
        factors_per_block : List[int]
            Number of factors per block, e.g., [5, 2, 2, 1, 1, 1]
        blocks_array : np.ndarray
            Binary array (N x n_blocks) where blocks_array[i, j] = 1 if series i loads on block j
        use_bias : bool
            Whether to use bias term
        """
        ...
        def __init__(
            ...
            """Initialize block-sparse decoder.
            ...
            blocks_array : np.ndarray
                Binary array (N x n_blocks) indicating which blocks each series loads on
            use_bias : bool
                Whether to use bias term
            """
            super().__init__()
            ...
            # Validate dimensions
            if sum(factors_per_block) != input_dim:
                raise ValueError(
                    f"Sum of factors_per_block ({sum(factors_per_block)}) must equal input_dim ({input_dim})"
                )
            if blocks_array.shape[0] != output_dim:
                raise ValueError(
                    f"blocks_array rows ({blocks_array.shape[0]}) must equal output_dim ({output_dim})"
                )
            if blocks_array.shape[1] != len(factors_per_block):
                raise ValueError(
                    f"blocks_array columns ({blocks_array.shape[1]}) must equal number of blocks ({len(factors_per_block)})"
                )
            
            self.factors_per_block = factors_per_block
            self.blocks_array = blocks_array
            
            # Create the linear layer (will be masked)
            self.decoder = nn.Linear(input_dim, output_dim, bias=use_bias)
            
            # Create block mask: (N x m) where mask[i, j] = 1 if series i can load on factor j
            # Factor indices: block 0 has factors [0:factors_per_block[0]],
            #                 block 1 has factors [factors_per_block[0]:factors_per_block[0]+factors_per_block[1]], etc.
            mask = np.zeros((output_dim, input_dim), dtype=np.float32)
            factor_start = 0
            for block_idx, n_factors in enumerate(factors_per_block):
                factor_end = factor_start + n_factors
                # Series that load on this block can load on these factors
                series_mask = blocks_array[:, block_idx:block_idx+1]  # (N, 1)
                mask[:, factor_start:factor_end] = series_mask  # Broadcast to all factors in this block
                factor_start = factor_end
            
            # Register as buffer (not a parameter, but part of model state)
            self.register_buffer('block_mask', torch.from_numpy(mask))
            
            # Initialize masked weights to zero (will be learned, but only in allowed positions)
            # Note: mask is already (output_dim, input_dim) = (N, m), matching decoder.weight shape
            with torch.no_grad():
                self.decoder.weight.data *= self.block_mask  # No transpose needed - mask already matches weight shape
        
        def forward(self, f: torch.Tensor) -> torch.Tensor:
            """Forward pass through block-sparse decoder.
            
            Applies the block mask to ensure only allowed connections are used.
            ...
            # Linear layer stores weights as (out_features, in_features) = (N, m)
            # Mask is (N, m), so we can multiply directly (no transpose needed)
            masked_weight = self.decoder.weight * self.block_mask
            return torch.nn.functional.linear(f, masked_weight, self.decoder.bias)
        
        def apply_block_mask(self):
            """Apply block mask to decoder weights (call after optimizer step).
            
            This ensures that weights outside allowed block connections are zeroed out.
