...
def save_failure_report(step: str, error: Exception, context: dict, project_root: Path):
    ...
    report_file = reports_dir / f"exchange_failure_{timestamp}.json"
...
@hydra.main(version_base=None, config_path="../config", config_name="dfm_exchange")
def main(cfg: DictConfig) -> None:
    ...
    try:
        print("\n[Step 1] Loading data...")
        try:
            df = pd.read_csv(project_root / "data" / "exchange_rate.csv")
        ...
        try:
            print("\n[Step 5] Saving model...")
            model_path = project_root / "models" / "dfm_exchange.pkl"
