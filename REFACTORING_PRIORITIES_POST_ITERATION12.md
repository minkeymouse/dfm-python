# Refactoring Priorities - Post Iteration 12
...
After 12 iterations, the codebase is **clean and well-organized**. Remaining opportunities focus on **file organization** rather than critical issues.
...
- ✅ Extracted Hydra registration from `config.py` to `config_sources.py` (50 lines)
...
**Remaining**: 1 MEDIUM priority task, 3 LOW priority tasks
...
- `_prepare_data_and_params()` (~90 lines) - Parameter resolution and data cleaning
- `_prepare_aggregation_structure()` (~55 lines) - Aggregation setup
- `_run_em_algorithm()` (~95 lines) - EM algorithm orchestration
- `_dfm_core()` (~235 lines) - Core DFM estimation

**Assessment**:
- Functions are DFM-specific (may not be reusable)
- Well-organized within `dfm.py`
- No immediate need to extract

**Action**:
- Only extract if functions become reusable
- Only extract if `dfm.py` grows significantly

**Recommendation**: **LOW Priority** - Monitor, only extract if beneficial.
